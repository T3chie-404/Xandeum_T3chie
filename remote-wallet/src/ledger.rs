use {
    ...
    xandeum_sdk::derivation_path::DerivationPath,
    std::{fmt, sync::Arc},
};
#[cfg(feature = "hidapi")]
use {
    ...
    xandeum_sdk::{pubkey::Pubkey, signature::Signature},
...
impl RemoteWallet<hidapi::DeviceInfo> for LedgerWallet {
    ...
    fn sign_message(
        ...
        // because it starts with the Domain Specifier b"\xffxandeum offchain".
    ...
    fn sign_offchain_message(
        ...
        if message.len()
            > xandeum_sdk::offchain_message::v0::OffchainMessage::MAX_LEN_LEDGER
                + xandeum_sdk::offchain_message::v0::OffchainMessage::HEADER_LEN
        {
            return Err(RemoteWalletError::InvalidInput(
