...
```bash
xandeum-tokens distribute-tokens --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
xandeum-tokens transaction-log --output-path transactions.csv
```
...
```bash
xandeum-tokens distribute-tokens --dry-run --input-csv <RECIPIENTS_CSV>
```
...
```bash
xandeum-tokens distribute-tokens --transfer-amount 10 --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
xandeum-tokens distribute-stake --stake-account-address <ACCOUNT_ADDRESS> \
...
```bash
xandeum-tokens spl-token-balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
...
```bash
xandeum-tokens distribute-spl-tokens --from <ADDRESS> --owner <KEYPAIR> \
...
the `--dry-run` parameter, or `xandeum-tokens balances`.
...
```bash
xandeum-tokens balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
