...
Operate a configured testnet
 ...
 sanity/start-specific options:
   ...
   -o noInstallCheck    - Skip xandeum-install sanity
 ...
 update-specific options:
   --platform linux|osx|windows       - Deploy the tarball using 'xandeum-install deploy ...' for the
...
startCommon() {
  ...
  local remoteSolanaHome="${remoteHome}/xandeum"
  ...
  [[ -z "$externalNodeSshKey" ]] || ssh-copy-id -f -i "$externalNodeSshKey" "${sshOptions[@]}" "xandeum@$ipAddress"
...
deployBootstrapValidator() {
  ...
  tar)
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$SOLANA_ROOT"/xandeum-release/bin/* "$ipAddress:$remoteCargoBin/"
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$SOLANA_ROOT"/xandeum-release/version.yml "$ipAddress:~/"
    ;;
  local)
...
startBootstrapLeader() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" -n "$ipAddress" \
      "./xandeum/net/remote/remote-node.sh \
         $deployMethod \
         bootstrap-validator \
         $entrypointIp \
...
startNode() {
  ...
  (
    ...
    if [[ $nodeType = blockstreamer ]] && [[ -n $letsEncryptDomainName ]]; then
      ...
      ssh "${sshOptions[@]}" -n "$ipAddress" \
        "sudo -H /certbot-restore.sh $letsEncryptDomainName maintainers@xandeumlabs.com"
...
startClient() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" -f "$ipAddress" \
      "./xandeum/net/remote/remote-client.sh $deployMethod $entrypointIp \
...
startClients() {
  for ((i=0; i < "$numClients" && i < "$numClientsRequested"; i++)) do
    if [[ $i -lt "$numBenchTpsClients" ]]; then
      startClient "${clientIpList[$i]}" "xandeum-bench-tps" "$i"
    else
...
sanity() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" "$bootstrapLeader" \
      "./xandeum/net/remote/remote-sanity.sh $bootstrapLeader $sanityExtraArgs \"$RUST_LOG\""
  ) || ok=false
  $ok || exit 1
  ...
  if [[ -z $skipBlockstreamerSanity && -n $blockstreamer ]]; then
    ...
    (
      ...
      ssh "${sshOptions[@]}" "$blockstreamer" \
        "./xandeum/net/remote/remote-sanity.sh $blockstreamer $sanityExtraArgs \"$RUST_LOG\""
    ) || ok=false
    $ok || exit 1
  fi
...
deployUpdate() {
  ...
  for updatePlatform in $updatePlatforms; do
    echo "--- Deploying xandeum-install update: $updatePlatform"
    (
      set -x

      scripts/xandeum-install-update-manifest-keypair.sh "$updatePlatform"

      timeout 30s scp "${sshOptions[@]}" \
        update_manifest_keypair.json "$bootstrapLeader:xandeum/update_manifest_keypair.json"

      # shellcheck disable=SC2029 # remote-deploy-update.sh args are expanded on client side intentionally
      ssh "${sshOptions[@]}" "$bootstrapLeader" \
        "./xandeum/net/remote/remote-deploy-update.sh $releaseChannel $updatePlatform"
    ) || ok=false
    $ok || exit 1
  done
}
...
prepareDeploy() {
  ...
  tar)
    if [[ -n $releaseChannel ]]; then
      ...
      rm -f "$SOLANA_ROOT"/xandeum-release.tar.bz2
      declare updateDownloadUrl=https://release.xandeum.com/"$releaseChannel"/xandeum-release-x86_64-unknown-linux-gnu.tar.bz2
      (
        set -x
        curl -L -I "$updateDownloadUrl"
        curl -L --retry 5 --retry-delay 2 --retry-connrefused \
          -o "$SOLANA_ROOT"/xandeum-release.tar.bz2 "$updateDownloadUrl"
      )
      tarballFilename="$SOLANA_ROOT"/xandeum-release.tar.bz2
    fi
    (
      set -x
      rm -rf "$SOLANA_ROOT"/xandeum-release
      cd "$SOLANA_ROOT"; tar jfxv "$tarballFilename"
      cat "$SOLANA_ROOT"/xandeum-release/version.yml
    )
    ;;
  local)
    if $doBuild; then
  ...
  *)
  ...
  if [[ -n $deployIfNewer ]]; then
    ...
    if ! diff -q current-version.yml "$SOLANA_ROOT"/xandeum-release/version.yml; then
...
deploy() {
  ...
  if ! $waitForNodeInit; then
    ...
    for ipAddress in "${validatorIpList[@]}" "${blockstreamerIpList[@]}"; do
      ...
      ssh "${sshOptions[@]}" -n "$ipAddress" \
        "./xandeum/net/remote/remote-node-wait-init.sh $((600 - timeWaited))"
    done
  fi
  ...
  tar)
    networkVersion="$(
      (
        set -o pipefail
        grep "^commit: " "$SOLANA_ROOT"/xandeum-release/version.yml | head -n1 | cut -d\  -f2
      ) || echo "tar-unknown"
    )"
    ;;
  local)
...
stopNode() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" "$ipAddress" "PS4=\"$PS4\" ./xandeum/net/remote/cleanup.sh"
  ) >> "$logFile" 2>&1 &
...
logs)
  ...
  fetchRemoteLog() {
    ...
    (
      set -x
      timeout 30s scp "${sshOptions[@]}" \
        "$ipAddress":xandeum/"$log".log "$netLogDir"/remote-"$log"-"$ipAddress".log
...
netem)
  if [[ -n $netemConfigFile ]]; then
    ...
    if [[ $netemCommand = "add" ]]; then
      for ipAddress in "${validatorIpList[@]}"; do
        remoteHome=$(remoteHomeDir "$ipAddress")
        remoteSolanaHome="${remoteHome}/xandeum"
        "$here"/scp.sh "$netemConfigFile" xandeum@"$ipAddress":"$remoteSolanaHome"
      done
    fi
    for i in "${!validatorIpList[@]}"; do
      "$here"/ssh.sh xandeum@"${validatorIpList[$i]}" 'xandeum/scripts/net-shaper.sh' \
      "$netemCommand" ~xandeum/xandeum/"$remoteNetemConfigFile" "${#validatorIpList[@]}" "$i"
    done
  else
    ...
    for ipAddress in "${validatorIpList[@]}"; do
      "$here"/ssh.sh xandeum@"$ipAddress" 'xandeum/scripts/netem.sh delete < xandeum/netem.cfg || true'
    done

    # Start netem on required nodes
    for ((i=0; i<num_nodes; i++ )); do :
      "$here"/ssh.sh xandeum@"${validatorIpList[$i]}" "echo $netemConfig > xandeum/netem.cfg; xandeum/scripts/netem.sh add \"$netemConfig\""
    done
  fi
  ;;
*)
