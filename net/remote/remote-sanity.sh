...
local|tar|skip)
  ...
  xandeum_cli=xandeum
  xandeum_gossip=xandeum-gossip
  xandeum_install=xandeum-install
  ;;
*)
...
(
  set -x
  $xandeum_cli --url http://"$sanityTargetIp":8899 validators
)
...
(
  ...
  $xandeum_gossip --allow-private-addr spy --entrypoint "$sanityTargetIp:8001" \
...
if $installCheck && [[ -r update_manifest_keypair.json ]]; then
  echo "--- $sanityTargetIp: xandeum-install test"

  (
    set -x
    rm -rf install-data-dir
    $xandeum_install init \
      --no-modify-path \
      --data-dir install-data-dir \
      --url http://"$sanityTargetIp":8899 \

    $xandeum_install info
  )
fi
