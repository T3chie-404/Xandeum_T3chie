...
spl() {
  (
    ...
    set -x
    rm -rf spl
    git clone https://github.com/xandeum-labs/xandeum-program-library.git spl
    # copy toolchain file to use xandeum's rust version
    cp "$SOLANA_DIR"/rust-toolchain.toml spl/
    cd spl || exit 1

    project_used_xandeum_version=$(sed -nE 's/xandeum-sdk = \"[>=<~]*(.*)\"/\1/p' <"token/program/Cargo.toml")
    echo "used xandeum version: $project_used_xandeum_version"
    if semverGT "$project_used_xandeum_version" "$SOLANA_VER"; then
      echo "skip"
      return
    fi
