...
FLAGS:
        ...
        --no-xandeum-prefix    Do not require \`xandeum-\` prefix on PACKAGE_NAME
...
if ${require_xandeum_prefix} && ! [[ "${package_name}" =~ ^xandeum- ]]; then
  # shellcheck disable=SC2016 # backticks are not a command here
  echo 'error: PACKAGE_NAME MUST start with `xandeum-`' 1>&2
  display_help
  exit 1
fi
...
license = "Apache-2.0"
homepage = "https://xandeumlabs.com"
