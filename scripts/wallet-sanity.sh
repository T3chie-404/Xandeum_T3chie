#!/usr/bin/env bash
#
# xandeum-cli integration sanity test
#
set -e
...
while [[ $timeout -gt 0 ]]; do
  set +e
  output=$($xandeum_cli "${args[@]}" transaction-count --commitment finalized)
  rc=$?
  set -e
...
(
  set -x
  $xandeum_cli "${args[@]}" address
  $xandeum_cli "${args[@]}" balance
  $xandeum_cli "${args[@]}" ping --count 5 --interval 0
  $xandeum_cli "${args[@]}" balance
)
