...
use {
    ...
    xandeum_cli_output::display::format_labeled_address,
    xandeum_metrics::{datapoint_error, datapoint_info},
    xandeum_notifier::{NotificationType, Notifier},
    xandeum_rpc_client::rpc_client::RpcClient,
    xandeum_rpc_client_api::{client_error, response::RpcVoteAccountStatus},
    xandeum_sdk::{
        hash::Hash,
...
fn get_config() -> Config {
    let matches = App::new(crate_name!())
        ...
        define environment variables before running `xandeum-watchtower`:
        ...
        define environment variable before running `xandeum-watchtower`:
        ...
        .arg(
            Arg::with_name("name_suffix")
                ...
                .takes_value(true)
                .default_value("")
                .help("Add this string into all notification messages after \"xandeum-watchtower\"")
        )
        .get_matches();
...
fn main() -> Result<(), Box<dyn error::Error>> {
    ...
    xandeum_metrics::set_panic_hook("watchtower", /*version:*/ None);
    ...
    loop {
        ...
        if let Some((failure_test_name, failure_error_message)) = &failure {
            ...
            if !last_notification_msg.is_empty() {
                ...
                info!("{}", all_clear_msg);
                notifier.send(
                    &format!("xandeum-watchtower{}: {}", config.name_suffix, all_clear_msg),
