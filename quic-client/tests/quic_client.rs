...
mod tests {
    use {
        ...
        log::*,
        xandeum_connection_cache::connection_cache_stats::ConnectionCacheStats,
        xandeum_perf::packet::PacketBatch,
        xandeum_quic_client::nonblocking::quic_client::{
            QuicClientCertificate, QuicLazyInitializedEndpoint,
        },
        xandeum_sdk::{net::DEFAULT_TPU_COALESCE, packet::PACKET_DATA_SIZE, signature::Keypair},
        xandeum_streamer::{
    ...
    fn test_quic_client_multiple_writes() {
        use {
            ...
            xandeum_quic_client::quic_client::QuicClientConnection,
        };
        xandeum_logger::setup();
        let (sender, receiver) = unbounded();
        let staked_nodes = Arc::new(RwLock::new(StakedNodes::default()));
        let (s, exit, keypair, ip) = server_args();
        let (_, t) = xandeum_streamer::quic::spawn_server(
            "quic_streamer_test",
            s.try_clone().unwrap(),
            &keypair,
            ip,
    ...
    async fn test_nonblocking_quic_client_multiple_writes() {
        use {
            xandeum_connection_cache::nonblocking::client_connection::ClientConnection,
            xandeum_quic_client::nonblocking::quic_client::QuicClientConnection,
        };
        xandeum_logger::setup();
        let (sender, receiver) = unbounded();
        let staked_nodes = Arc::new(RwLock::new(StakedNodes::default()));
        let (s, exit, keypair, ip) = server_args();
        let (_, _, t) = xandeum_streamer::nonblocking::quic::spawn_server(
            "quic_streamer_test",
            s.try_clone().unwrap(),
            &keypair,
            ip,
    ...
    fn test_quic_bi_direction() {
        ...
        let (request_recv_endpoint, request_recv_thread) = xandeum_streamer::quic::spawn_server(
            "quic_streamer_test",
