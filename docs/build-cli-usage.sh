...
echo 'Building the xandeum cli from source...'
...
cargo build -p xandeum-cli

usage=$(cargo -q run -p xandeum-cli -- -C ~/.foo --help | sed -e 's|'"$HOME"'|~|g' -e 's/[[:space:]]\+$//')
...
while read -r subcommand rest; do
  ...
  if ((in_subcommands)); then
      section "$(cargo -q run -p xandeum-cli -- help "$subcommand" | sed -e 's|'"$HOME"'|~|g' -e 's/[[:space:]]\+$//')" "####" >> "$out"
  fi
done <<<"$usage">>"$out"
