...
```bash
$ npm install --save @xandeum/web3.js
```
...
<script src="https://unpkg.com/@xandeum/web3.js@latest/lib/index.iife.js"></script>

<!-- Production (minified) -->
<script src="https://unpkg.com/@xandeum/web3.js@latest/lib/index.iife.min.js"></script>
```
...
```javascript
const xandeumWeb3 = require("@xandeum/web3.js");
console.log(xandeumWeb3);
```

#### ES6

```javascript
import * as xandeumWeb3 from "@xandeum/web3.js";
console.log(xandeumWeb3);
```
...
```javascript
// xandeumWeb3 is provided in the global namespace by the bundle script
console.log(xandeumWeb3);
```
...
Let's look at how to call this instruction using xandeum-web3.js:
...
```rust
/* https://github.com/xandeum-labs/xandeum/blob/21bc43ed58c63c827ba4db30426965ef3e807180/sdk/program/src/system_instruction.rs#L142-L305 */
...
`ns64('space')` is the argument for the `allocate` function. You can see in the original `allocate` function in Rust that space was of the type `u64`. `u64` is an unsigned 64bit integer. Javascript by default only provides up to 53bit integers. `ns64` comes from `@xandeum/buffer-layout` to help with type conversions between Rust and Javascript. You can find more type conversions between Rust and Javascript at [xandeum-labs/buffer-layout](https://github.com/xandeum-labs/buffer-layout).
...
```javascript
const { struct, u32, ns64 } = require("@xandeum/buffer-layout");
