...
```bash
xandeum-keygen new -o nonce-keypair.json
xandeum create-nonce-account nonce-keypair.json 1
```
...
> [Full usage documentation](../cli/usage.md#xandeum-create-nonce-account)
...
```bash
xandeum nonce nonce-keypair.json
```
...
> [Full usage documentation](../cli/usage.md#xandeum-get-nonce)
...
```bash
xandeum new-nonce nonce-keypair.json
```
...
> [Full usage documentation](../cli/usage.md#xandeum-new-nonce)
...
```bash
xandeum nonce-account nonce-keypair.json
```
...
> [Full usage documentation](../cli/usage.md#xandeum-nonce-account)
...
```bash
xandeum withdraw-from-nonce-account nonce-keypair.json ~/.config/xandeum/id.json 0.5
```
...
> [Full usage documentation](../cli/usage.md#xandeum-withdraw-from-nonce-account)
...
```bash
xandeum authorize-nonce-account nonce-keypair.json nonce-authority.json
```
...
> [Full usage documentation](../cli/usage.md#xandeum-authorize-nonce-account)
...
```bash
$ xandeum-keygen new -o alice.json
$ xandeum-keygen new -o nonce.json
$ xandeum-keygen new -o bob.json
```
...
```bash
$ xandeum airdrop -k alice.json 1
1 SOL
```
...
```bash
$ xandeum create-nonce-account -k alice.json nonce.json 0.1
...
```bash
$ xandeum transfer -k alice.json --blockhash expiredDTaxfagttWjQweib42b6ZHADSx94Tw8gHx11 bob.json 0.01
[2020-01-02T18:48:28.462911000Z ERROR xandeum_cli::cli] Io(Custom { kind: Other, error: "Transaction \"33gQQaoPc9jWePMvDAeyJpcnSPiGUAdtVg8zREWv4GiKjkcGNufgpcbFyRKRrA25NkgjZySEeKue5rawyeH5TzsV\" failed: None" })
...
```bash
$ xandeum nonce-account nonce.json
balance: 0.1 SOL
minimum balance required: 0.00136416 SOL
...
```bash
$ xandeum transfer -k alice.json --blockhash F7vmkY3DTaxfagttWjQweib42b6ZHADSx94Tw8gHx3W7 --nonce nonce.json bob.json 0.01
