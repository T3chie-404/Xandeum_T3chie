...
[Staking and Inflation FAQ](https://xandeum.com/staking).
...
```bash
xandeum-keygen new --no-passphrase -o stake-account.json
```
...
```bash
xandeum create-stake-account --from <KEYPAIR> stake-account.json <AMOUNT> \
...
View the new stake account with the `xandeum stake-account` command:
...
`xandeum stake-authorize` command. For example, to set a new stake authority,
run:

```bash
xandeum stake-authorize <STAKE_ACCOUNT_ADDRESS> \
...
```bash
xandeum create-stake-account --from <KEYPAIR> <STAKE_ACCOUNT_KEYPAIR> --seed <STRING> <AMOUNT> \
...
and seed string. To see what stake address the command will derive, use `xandeum create-address-with-seed`:

```bash
xandeum create-address-with-seed --from <PUBKEY> <SEED_STRING> STAKE
```

`<PUBKEY>` is the public key of the `<STAKE_ACCOUNT_KEYPAIR>` passed to
`xandeum create-stake-account`.
...
accounts with the `xandeum validators` command:
...
address in `xandeum delegate-stake`:

```bash
xandeum delegate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <VOTE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
After delegating stake, use `xandeum stake-account` to observe the changes
...
Once delegated, you can undelegate stake with the `xandeum deactivate-stake`
command:

```bash
xandeum deactivate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
Transfer tokens out of a stake account with the `xandeum withdraw-stake` command:

```bash
xandeum withdraw-stake --withdraw-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <RECIPIENT_ADDRESS> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
...
existing stake account to a new one, use the `xandeum split-stake` command:

```bash
xandeum split-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <NEW_STAKE_ACCOUNT_KEYPAIR> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
