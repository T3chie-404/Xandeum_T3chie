...
```bash
xandeum program deploy <PROGRAM_FILEPATH>
```
...
```bash
xandeum program deploy --program-id <KEYPAIR_FILEPATH> <PROGRAM_FILEPATH>
```
...
```bash
xandeum program show <ACCOUNT_ADDRESS>
```
...
```bash
xandeum program deploy --max-len 200000 <PROGRAM_FILEPATH>
```
...
`xandeum-keygen recover` and the following 12-word seed phrase:
==================================================================================
valley flat great hockey share token excess clever benefit traffic avocado athlete
==================================================================================
To resume a deploy, pass the recovered keypair as
the [BUFFER_SIGNER] to `xandeum program deploy` or `xandeum program write-buffer'.
Or to recover the account's lamports, pass it as the
[BUFFER_ACCOUNT_ADDRESS] argument to `xandeum program drain`.
==================================================================================
```
...
```bash
xandeum-keygen recover -o <KEYPAIR_PATH>
```
...
```bash
xandeum program deploy --buffer <KEYPAIR_PATH> <PROGRAM_FILEPATH>
```
...
```bash
xandeum program show --programs
xandeum program show --buffers
```

To specify a different authority:

```bash
xandeum program show --programs --buffer-authority <AURTHORITY_ADRESS>
xandeum program show --buffers --buffer-authority <AURTHORITY_ADRESS>
```

To close a single account:

```bash
xandeum program close <BADDRESS>
```

To close a single account and specify a different authority than the default:

```bash
xandeum program close <ADDRESS> --buffer-authority <KEYPAIR_FILEPATH>
```

To close a single account and specify a different recipient than the default:

```bash
xandeum program close <ADDRESS> --recipient <RECIPIENT_ADDRESS>
```

To close all the buffer accounts associated with the current authority:

```bash
xandeum program close --buffers
```

To show all buffer accounts regardless of the authority

```bash
xandeum program show --buffers --all
```
...
```bash
xandeum program deploy --upgrade-authority <UPGRADE_AUTHORITY_SIGNER> <PROGRAM_FILEPATH>
```

Or after deployment and using the default keypair as the current authority:

```bash
xandeum program set-upgrade-authority <PROGRAM_ADDRESS> --new-upgrade-authority <NEW_UPGRADE_AUTHORITY>
```

Or after deployment and specifying the current authority:

```bash
xandeum program set-upgrade-authority <PROGRAM_ADDRESS> --upgrade-authority <UPGRADE_AUTHORITY_SIGNER> --new-upgrade-authority <NEW_UPGRADE_AUTHORITY>
```
...
```bash
xandeum program deploy <PROGRAM_FILEPATH> --final
```

Or anytime after:

```bash
xandeum program set-upgrade-authority <PROGRAM_ADDRESS> --final
```
...
```bash
xandeum program dump <ACCOUNT_ADDRESS> <OUTPUT_FILEPATH>
```
...
```bash
$ xandeum dump <ACCOUNT_ADDRESS> dump.so
...
```bash
xandeum program write-buffer <PROGRAM_FILEPATH>
```

Buffer accounts support authorities like program accounts:

```bash
xandeum program set-buffer-authority <BUFFER_ADDRESS> --new-buffer-authority <NEW_BUFFER_AUTHORITY>
```
...
```bash
xandeum program deploy --program-id <PROGRAM_ADDRESS> --buffer <BUFFER_ADDRESS>
```
