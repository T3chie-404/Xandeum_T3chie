...
Use Solana's command-line tool `xandeum-keygen` to generate keypair files. For
example, run the following from a command-line shell:

```bash
mkdir ~/my-xandeum-wallet
xandeum-keygen new --outfile ~/my-xandeum-wallet/my-keypair.json
```
...
```bash
xandeum-keygen pubkey ~/my-xandeum-wallet/my-keypair.json
```
...
`~/my-xandeum-wallet/my-keypair.json`. The public key of the keypair file is
...
```bash
xandeum-keygen verify <PUBKEY> ~/my-xandeum-wallet/my-keypair.json
```
