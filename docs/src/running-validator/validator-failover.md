...
### Primary Validator
The following additional `xandeum-validator` parameters are required to enable
...
following `xandeum-validator` command-line argument changes:
...
```bash
$ xandeum-validator wait-for-restart-window --identity validator-keypair.json \
  && xandeum-validator set-identity validator-keypair.json
```
...
Monitoring of your choosing can invoke the `xandeum-validator set-identity
