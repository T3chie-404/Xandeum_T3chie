...
[create-vote-account](../cli/usage.md#xandeum-create-vote-account) command.
...
the [vote-update-validator](../cli/usage.md#xandeum-vote-update-validator) command.
...
[vote-authorize-voter-checked](../cli/usage.md#xandeum-vote-authorize-voter-checked) command.

The vote authority can be changed at most once per epoch. If the authority is
changed with [vote-authorize-voter-checked](../cli/usage.md#xandeum-vote-authorize-voter-checked),
this will not take effect until the beginning of the next epoch.
To support a smooth transition of the vote signing,
`xandeum-validator` allows the `--authorized-voter` argument to be specified
...
using the [withdraw-from-vote-account](../cli/usage.md#xandeum-withdraw-from-vote-account)
...
[vote-update-commission](../cli/usage.md#xandeum-vote-update-commission) command.
...
1. Create the new validator identity keypair, `xandeum-keygen new -o ~/new-validator-keypair.json`.
2. Ensure that the new identity account has been funded, `xandeum transfer ~/new-validator-keypair.json 500`.
3. Run `xandeum vote-update-validator ~/vote-account-keypair.json ~/new-validator-keypair.json ~/authorized_withdrawer.json`
...
no longer listed in the `xandeum leader-schedule` output.

### Vote Account Authorized Voter

The _vote authority_ keypair may only be changed at epoch boundaries and
requires some additional arguments to `xandeum-validator` for a seamless
migration.

1. Run `xandeum epoch-info`. If there is not much time remaining time in the
   current epoch, consider waiting for the next epoch to allow your validator
   plenty of time to restart and catch up.
2. Create the new vote authority keypair, `xandeum-keygen new -o ~/new-vote-authority.json`.
3. Determine the current _vote authority_ keypair by running `xandeum vote-account ~/vote-account-keypair.json`. It may be validator's
   identity account (the default) or some other keypair. The following steps
   assume that `~/validator-keypair.json` is that keypair.
4. Run `xandeum vote-authorize-voter-checked ~/vote-account-keypair.json ~/validator-keypair.json ~/new-vote-authority.json`.
   The new vote authority is scheduled to become active starting at the next epoch.
5. `xandeum-validator` now needs to be restarted with the old and new vote
...
6. After the cluster reaches the next epoch, remove the
   ...
   `xandeum-validator`, as the old vote authority keypair is no longer required.

### Vote Account Authorized Withdrawer

No special handling or timing considerations are required.
Use the `xandeum vote-authorize-withdrawer-checked` command as needed.
...
1. Entity B creates a durable nonce using `xandeum create-nonce-account`
2. Entity B then runs a `xandeum vote-authorize-voter-checked` or `xandeum vote-authorize-withdrawer-checked` command, including:
...
3. When the `xandeum vote-authorize-...-checked` command successfully executes, it will output transaction signatures that Entity B must share with Entity A
4. Entity A then runs a similar `xandeum vote-authorize-voter-checked` or `xandeum vote-authorize-withdrawer-checked` command with the following changes:
...
[close-vote-account](../cli/usage.md#xandeum-close-vote-account) command.
