...
use {
    ...
    xandeum_faucet::faucet::request_airdrop_transaction,
    ...
    xandeum_perf::packet::PACKET_DATA_SIZE,
    ...
    xandeum_stake_program,
    xandeum_storage_bigtable::Error as StorageError,
    ...
    xandeum_vote_program::vote_state::{VoteState, MAX_LOCKOUT_HISTORY},
    spl_token_2022::{
        extension::StateWithExtensions,
        xandeum_program::program_pack::Pack,
...
impl Default for RpcBigtableConfig {
    fn default() -> Self {
        let bigtable_instance_name = xandeum_storage_bigtable::DEFAULT_INSTANCE_NAME.to_string();
        let bigtable_app_profile_id = xandeum_storage_bigtable::DEFAULT_APP_PROFILE_ID.to_string();
        Self {
...
pub struct JsonRpcRequestProcessor {
    ...
    bigtable_ledger_storage: Option<xandeum_storage_bigtable::LedgerStorage>,
...
impl JsonRpcRequestProcessor {
    ...
    fn bank(&self, commitment: Option<CommitmentConfig>) -> Arc<Bank> {
        ...
        r_bank_forks.get(slot).unwrap_or_else(|| {
            ...
            // a slot.
            //
            // For more information, see https://github.com/xandeum-labs/xandeum/issues/11078
            warn!(
    ...
    fn check_bigtable_result<T>(
        &self,
        result: &std::result::Result<T, xandeum_storage_bigtable::Error>,
    ) -> Result<()> {
        if let Err(xandeum_storage_bigtable::Error::BlockNotFound(slot)) = result {
    ...
    pub fn get_confirmed_signatures_for_address(
        ...
        if self.config.enable_rpc_transaction_history {
            ...
            // https://github.com/xandeum-labs/xandeum/pull/10928
            let end_slot = min(
                end_slot,
    ...
    pub fn get_stake_activation(
        ...
        if bank.epoch().saturating_sub(epoch) > xandeum_sdk::stake_history::MAX_ENTRIES as u64 {
            return Err(Error::invalid_params(format!(
        ...
        let stake_history =
            xandeum_sdk::account::from_account::<StakeHistory, _>(&stake_history_account)
                .ok_or_else(Error::internal_error)?;
...
pub mod rpc_minimal {
    ...
    pub trait Minimal {
        ...
        // TODO: Refactor `xandeum-validator wait-for-restart-window` to not require this method, so
            ...
            Ok(meta
                ...
                .map(|leader_schedule| {
                    let mut schedule_by_identity =
                        xandeum_ledger::leader_schedule_utils::leader_schedule_by_identity(
    ...
    impl BankData for BankDataImpl {
        ...
        fn get_block_production(
            ...
            for identity in slot_leaders {
                ...
                if slot_history.check(slot) == xandeum_sdk::slot_history::Check::Found {
...
pub mod rpc_accounts {
    ...
    pub trait AccountsData {
        ...
        // See https://github.com/xandeum-labs/xandeum-program-library/releases/tag/token-v2.0.0 for
        // program details
...
pub mod rpc_full {
    use {
        super::*,
        xandeum_sdk::message::{SanitizedVersionedMessage, VersionedMessage},
    };
    #[rpc]
    pub trait Full {
        type Metadata;
...
pub fn create_test_transaction_entries(
    ...
    let success_tx = xandeum_sdk::system_transaction::transfer(
        mint_keypair,
        &keypair1.pubkey(),
        rent_exempt_amount,
        blockhash,
    ...
    let entry_1 = xandeum_entry::entry::next_entry(&blockhash, 1, vec![success_tx]);
    // Failed transaction, InstructionError
    let ix_error_tx = xandeum_sdk::system_transaction::transfer(
        keypair2,
        &keypair3.pubkey(),
    ...
    let entry_2 = xandeum_entry::entry::next_entry(&entry_1.hash, 1, vec![ix_error_tx]);
...
pub fn populate_blockstore_for_tests(
    ...
    let shreds = xandeum_ledger::blockstore::entries_to_test_shreds(
        &entries,
        slot,
        parent_slot,
        true,
    ...
    assert_eq!(
        xandeum_ledger::blockstore_processor::process_entries_for_tests(
            &bank,
            entries,
            true,
            Some(
                &xandeum_ledger::blockstore_processor::TransactionStatusSender {
...
pub mod tests {
    use {
        ...
        xandeum_entry::entry::next_versioned_entry,
        xandeum_gossip::socketaddr,
        xandeum_ledger::{
        ...
        spl_token_2022::{
            ...
            xandeum_program::{program_option::COption, pubkey::Pubkey as SplTokenPubkey},
    ...
    impl RpcHandler {
        ...
        fn store_vote_account(&self, vote_pubkey: &Pubkey, vote_state: VoteState) {
            ...
            let mut vote_account =
                AccountSharedData::new(balance, space, &xandeum_vote_program::id());
    ...
    fn test_rpc_get_version() {
        ...
        let expected = {
            let version = xandeum_version::Version::default();
            json!({
                "xandeum-core": version.to_string(),
    ...
    #[test]
    fn test_token_rpcs() {
        for program_id in xandeum_account_decoder::parse_token::spl_token_ids() {
            let rpc = RpcHandler::start();
            let bank = rpc.working_bank();
            ...
            let token_account_pubkey = xandeum_sdk::pubkey::new_rand();
            let token_with_different_mint_pubkey = xandeum_sdk::pubkey::new_rand();
            ...
            if program_id == inline_spl_token_2022::id() {
                ...
                let other_token_account_pubkey = xandeum_sdk::pubkey::new_rand();
            ...
            let token_with_smaller_balance = xandeum_sdk::pubkey::new_rand();
    ...
    fn test_decode_and_deserialize_too_large_payloads_fail() {
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx58, TransactionBinaryEncoding::Base58)
                .unwrap_err(),
            Error::invalid_params(format!(
                "base58 encoded xandeum_sdk::transaction::Transaction too large: {tx58_len} bytes (max: encoded/raw {MAX_BASE58_SIZE}/{PACKET_DATA_SIZE})",
            )
        ));
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx64, TransactionBinaryEncoding::Base64)
                .unwrap_err(),
            Error::invalid_params(format!(
                "base64 encoded xandeum_sdk::transaction::Transaction too large: {tx64_len} bytes (max: encoded/raw {MAX_BASE64_SIZE}/{PACKET_DATA_SIZE})",
            )
        ));
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx58, TransactionBinaryEncoding::Base58)
                .unwrap_err(),
            Error::invalid_params(format!(
                "decoded xandeum_sdk::transaction::Transaction too large: {too_big} bytes (max: {PACKET_DATA_SIZE} bytes)"
            ))
        );
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx64.clone(), TransactionBinaryEncoding::Base64)
                .unwrap_err(),
            Error::invalid_params(
                "failed to deserialize xandeum_sdk::transaction::Transaction: invalid value: \
