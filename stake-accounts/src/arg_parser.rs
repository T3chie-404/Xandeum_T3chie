use {
    ...
    xandeum_cli_config::CONFIG_FILE,
    xandeum_sdk::native_token::sol_to_lamports,
    std::{ffi::OsString, process::exit},
};

fn fee_payer_arg<'a, 'b>() -> Arg<'a, 'b> {
    xandeum_clap_utils::fee_payer::fee_payer_arg().required(true)
}
    ...
    App::new(crate_name!())
        ...
        .arg(
            Arg::with_name("url")
                ...
                .takes_value(true)
                .value_name("URL")
                .help("RPC entrypoint address. i.e. http://api.devnet.xandeum.com"),
        )
        .subcommand(
