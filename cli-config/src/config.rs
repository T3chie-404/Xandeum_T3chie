...
lazy_static! {
    ...
    pub static ref CONFIG_FILE: Option<String> = {
        dirs_next::home_dir().map(|mut path| {
            path.extend([".config", "xandeum", "cli", "config.yml"]);
...
pub struct Config {
    ...
    ///
    /// [rpcdocs]: https://docs.xandeum.com/cluster/rpc-endpoints
    ...
    /// documentation for `xandeum_clap_utils::keypair::signer_from_path`.
    ...
    /// function, or with `xandeum_clap_utils::keypair::DefaultSigner`.
    ...
    /// `xandeum_sdk::commitment_config::CommitmentLevel::Confirmed`.
    #[serde(default)]
...
impl Default for Config {
    fn default() -> Self {
        let keypair_path = {
            ...
            keypair_path.extend([".config", "xandeum", "id.json"]);
            keypair_path.to_str().unwrap().to_string()
        };
        let json_rpc_url = "https://api.mainnet-beta.xandeum.com".to_string();
...
mod test {
    ...
    fn compute_websocket_url() {
        assert_eq!(
            Config::compute_websocket_url("http://api.devnet.xandeum.com"),
            "ws://api.devnet.xandeum.com/".to_string()
        );

        assert_eq!(
            Config::compute_websocket_url("https://api.devnet.xandeum.com"),
            "wss://api.devnet.xandeum.com/".to_string()
        );
