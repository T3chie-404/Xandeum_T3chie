#!/usr/bin/env bash
#
# Fetches the latest SPL programs and produces the xandeum-genesis command-line
...
fetch_program() {
  ...
  if [[ -r ~/.cache/xandeum-spl/$so ]]; then
    cp ~/.cache/xandeum-spl/"$so" "$so"
  else
    ...
    (
      ...
      curl -L --retry 5 --retry-delay 2 --retry-connrefused \
        -o "$so" \
        "https://github.com/xandeum-labs/xandeum-program-library/releases/download/$name-v$version/$so_name"
    )

    mkdir -p ~/.cache/xandeum-spl
    cp "$so" ~/.cache/xandeum-spl/"$so"
  fi
...
echo
echo "xandeum-genesis command-line arguments (spl-genesis-args.sh):"
