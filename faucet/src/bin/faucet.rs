use {
    ...
    log::*,
    xandeum_clap_utils::input_parsers::{lamports_of_sol, value_of},
    xandeum_faucet::{
        faucet::{run_faucet, Faucet, FAUCET_PORT},
        socketaddr,
    },
    xandeum_sdk::signature::read_keypair_file,
    std::{
        collections::HashSet,
...
#[tokio::main]
async fn main() {
    let default_keypair = xandeum_cli_config::Config::default().keypair_path;

    xandeum_logger::setup_with_default("xandeum=info");
    xandeum_metrics::set_panic_hook("faucet", /*version:*/ None);
