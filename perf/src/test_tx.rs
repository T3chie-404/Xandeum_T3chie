    ...
    R: CryptoRng + RngCore,
{
    ...
    slots.sort_unstable();
    slots.dedup();
    let switch_proof_hash = rng.gen_bool(0.5).then(|| xandeum_sdk::hash::new_rand(rng));
    vote_transaction::new_vote_transaction(
        slots,
        xandeum_sdk::hash::new_rand(rng), // bank_hash
        xandeum_sdk::hash::new_rand(rng), // blockhash
